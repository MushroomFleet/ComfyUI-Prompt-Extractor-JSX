...
- **Non-destructive parsing** — reads PNG chunk metadata without modifying image data
- **Zero dependencies** — uses native browser APIs only
- **Drag-and-drop support** — intuitive file upload experience
- **Image preview** — displays uploaded image alongside extracted prompt
- **One-click download** — save extracted prompt as `.txt` with matching filename
- **Full metadata access** — exposes complete workflow and prompt JSON
- **TypeScript ready** — includes type definitions for TS projects

## 🚀 Quick Start

### Try the Demo

Open `demo.html` in your browser for an instant preview. No build tools required — it uses Babel standalone for JSX transformation.
...
The image data itself is never modified — only the metadata header is read.
...
ComfyUI-Prompt-Extractor-JSX/
├── README.md                                    # This file
├── comfyprompt-extractor.jsx                   # Main component
├── demo.html                                    # Interactive demo
├── test_comfyui_image.png                      # Test file with metadata
└── ComfyUI-Prompt-Extractor-JSX-integration.md # Developer guide
```
